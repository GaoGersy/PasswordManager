...
public class PasswordBean extends RealmObject implements Comparable<PasswordBean> {
    @PrimaryKey
    private long id;
    ...
    public void copyParams(PasswordBean bean) {
...
//            this.updateHistorys = bean.getUpdateHistorys();
    ...
    @Override
    public int compareTo(PasswordBean bean) {//按照从小到大排列
        if (bean.index > index) {
            return -1;
