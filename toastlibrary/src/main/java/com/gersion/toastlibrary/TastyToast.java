package com.gersion.toastlibrary;

import android.content.Context;
import android.graphics.Color;
import android.view.LayoutInflater;
import android.view.View;
import android.widget.TextView;
import android.widget.Toast;

import com.facebook.rebound.SimpleSpringListener;
import com.facebook.rebound.Spring;
import com.facebook.rebound.SpringConfig;
import com.facebook.rebound.SpringSystem;
import com.sdsmdg.tastytoast.R;

/**
 * Created by rahul on 28/7/16.
 */
public class TastyToast {
    public static final int LENGTH_SHORT = 0;
    public static final int LENGTH_LONG = 1;


    public static final int SUCCESS = 1;
    public static final int WARNING = 2;
    public static final int ERROR = 3;
    public static final int INFO = 4;
    public static final int DEFAULT = 5;

    static SuccessToastView successToastView;
    static WarningToastView warningToastView;
    static ErrorToastView errorToastView;
    static InfoToastView infoToastView;
    static DefaultToastView defaultToastView;
    private static Toast mToast;
    private static TextView mTextS;
    private static Context mContext;
    private static int mType;
    private static TextView mTextE;
    private static TextView mTextI;
    private static TextView mTextD;
    private static TextView mTextW;

    public static void makeText(Context context, String msg, int length, int type) {
        mContext = context;
        mType = type;
        mToast = new Toast(context);


        switch (type) {
            case 1: {
                View layout = LayoutInflater.from(context).inflate(R.layout.success_toast_layout, null, false);
                mTextS = (TextView) layout.findViewById(R.id.toastMessage);
                mTextS.setText(msg);
                successToastView = (SuccessToastView) layout.findViewById(R.id.successView);
                successToastView.startAnim();
                mTextS.setBackgroundResource(R.drawable.success_toast);
                mTextS.setTextColor(Color.parseColor("#FFFFFF"));
                mToast.setView(layout);
                break;
            }
            case 2: {
                View layout = LayoutInflater.from(context).inflate(R.layout.warning_toast_layout, null, false);

                mTextW = (TextView) layout.findViewById(R.id.toastMessage);
                mTextW.setText(msg);

                warningToastView = (WarningToastView) layout.findViewById(R.id.warningView);
                SpringSystem springSystem = SpringSystem.create();
                final Spring spring = springSystem.createSpring();
                spring.setCurrentValue(1.8);
                SpringConfig config = new SpringConfig(40, 5);
                spring.setSpringConfig(config);
                spring.addListener(new SimpleSpringListener() {

                    @Override
                    public void onSpringUpdate(Spring spring) {
                        float value = (float) spring.getCurrentValue();
                        float scale = (float) (0.9f - (value * 0.5f));

                        warningToastView.setScaleX(scale);
                        warningToastView.setScaleY(scale);
                    }
                });
                Thread t = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            Thread.sleep(500);
                        } catch (InterruptedException e) {
                        }
                        spring.setEndValue(0.4f);
                    }
                });

                t.start();
                mTextW.setBackgroundResource(R.drawable.warning_toast);
                mTextW.setTextColor(Color.parseColor("#FFFFFF"));
                mToast.setView(layout);
                break;
            }
            case 3: {
                View layout = LayoutInflater.from(context).inflate(R.layout.error_toast_layout, null, false);

                mTextE = (TextView) layout.findViewById(R.id.toastMessage);
                mTextE.setText(msg);
                errorToastView = (ErrorToastView) layout.findViewById(R.id.errorView);
                errorToastView.startAnim();
                mTextE.setBackgroundResource(R.drawable.error_toast);
                mTextE.setTextColor(Color.parseColor("#FFFFFF"));
                mToast.setView(layout);
                break;
            }
            case 4: {
                View layout = LayoutInflater.from(context).inflate(R.layout.info_toast_layout, null, false);

                mTextI = (TextView) layout.findViewById(R.id.toastMessage);
                mTextI.setText(msg);
                infoToastView = (InfoToastView) layout.findViewById(R.id.infoView);
                infoToastView.startAnim();
                mTextI.setBackgroundResource(R.drawable.info_toast);
                mTextI.setTextColor(Color.parseColor("#FFFFFF"));
                mToast.setView(layout);
                break;
            }
            case 5: {
                View layout = LayoutInflater.from(context).inflate(R.layout.default_toast_layout, null, false);

                mTextD = (TextView) layout.findViewById(R.id.toastMessage);
                mTextD.setText(msg);
                defaultToastView = (DefaultToastView) layout.findViewById(R.id.defaultView);
                defaultToastView.startAnim();
                mTextD.setBackgroundResource(R.drawable.default_toast);
                mTextD.setTextColor(Color.parseColor("#FFFFFF"));
                mToast.setView(layout);
                break;
            }
        }
        mToast.setDuration(length);
    }

    public void show() {
        mToast.show();
    }

    public void setText(String text) {
        TextView textView = new TextView(mContext);
        switch (mType) {
            case SUCCESS:
                textView = mTextS;
                break;
            case WARNING:
                textView = mTextW;
                break;
            case ERROR:
                textView = mTextE;
                break;
            case INFO:
                textView = mTextI;
                break;
            case DEFAULT:
                textView = mTextD;
                break;

        }
        textView.setText(text);
    }

}
